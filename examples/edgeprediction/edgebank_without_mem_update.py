...
# set EdgeBank without memory update
...
edgebank = EdgeBankPredictor(history_src=tr_val_hist_src, history_dst=tr_val_hist_dst, history_ts=tr_val_hist_ts,
                            memory_mode=edgebank_memory_mode, time_window_ratio=time_window_ratio)
...
def test_one_vs_many(loader, neg_sampler, split_mode):
    ...
    for pos_batch in tqdm(loader):
        ...
        for idx, neg_batch in enumerate(neg_batch_list):
            ...
            y_pred = edgebank.predict_link_proba(query_src, query_dst)
            # compute hist@k & MRR
