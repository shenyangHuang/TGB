...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, model_name, static_graph, mode, split_mode):
    ...
    model.eval()
    input_list = [snap for snap in history_list[-args.history_len:]] 
    perf_list_all = []
...
# TODO: what to do about this part: 
    ...
    # create stat
    model = RecurrentRGCN(args.decoder, #TODO: this has slightly different args than CEN
                          args.encoder,
                        num_nodes,
                        num_rels,
...
# single run
    ...
    args.test = False
    val_mrr, _ = run_experiment(args)
    start_test = timeit.default_timer()
    args.test = True
    test_mrr, _ = run_experiment(args)
