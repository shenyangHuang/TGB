...
def test_tvi(data, test_mask, neg_sampler, split_mode, nodebank):
    r"""
    ...
    transductive_metrics = float(np.mean(transductive_list))
    inductive_metrics = float(np.mean(inductive_list))
...
print(f"\ttrain: Elapsed Time (s): {train_time: .4f}")
...
trans_mrr, ind_mrr = test_tvi(data, val_mask, neg_sampler, split_mode='val', nodebank=nodebank)
end_val = timeit.default_timer()

print(f"INFO: val: Evaluation Setting: >>> ONE-VS-MANY <<< ")
print(f"\tval transductive: {metric}: {trans_mrr: .4f}")
print(f"\tval inductive: {metric}: {ind_mrr: .4f}")
...
trans_mrr, ind_mrr = test_tvi(data, test_mask, neg_sampler, split_mode='test', nodebank=nodebank)
end_test = timeit.default_timer()

print(f"INFO: Test: Evaluation Setting: >>> ONE-VS-MANY <<< ")
print(f"\tTest transductive: {metric}: {trans_mrr: .4f}")
print(f"\tTest inductive: {metric}: {ind_mrr: .4f}")
