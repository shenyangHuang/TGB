...
def force_download_all():
    ...
    print(
        BColors.WARNING
        + "attempting download all data, will remove ALL files and download ALL possible files"
        + BColors.ENDC
    )
...
def unzip_delete():
    ...
    if not os.path.exists("./TG_network_datasets.zip"):
        print(
            f"{BColors.FAIL}DOWNLOAD FAILED{BColors.ENDC}, TG_network_datasets not found"
        )
        return
...
class TemporalDataSets(object):
    ...
    def __init__(
        ...
        self.exception_msg_process = (
            BColors.FAIL
            + "please run process() method before retrieving training data"
            + BColors.ENDC
        )
        ...
        if data_name not in check_dict.keys():
            sys.stdout.write(
                f" input for TemporalDataSets: '{str(data_name)}' not found, \n inputs must be "
            ...
            if inp == "y":
                ...
                sys.stdout.write(
                    BColors.WARNING
                    + "program will continue but program is unsafe \n"
                    + BColors.ENDC
                )
    ...
    def check_downloaded(self):
        ...
        if not osp.isdir(f"./{self.data_root}"):
            print(
                f"{BColors.FAIL}folder {self.data_root} not found/ Data not found{BColors.ENDC}"
            )
            self.download_file()
            return
        list_data_not_found = []
        ...
        if not list_data_not_found:
            if self.data_set_statistics:
                print(
                    f"All data found for {BColors.OKGREEN}{self.data_str}{BColors.ENDC}"
                )
        else:
    ...
    def process(self):
        ...
        # }
        data_splits = self.get_data_link_pred_from_indices(
            self.data_str, split_masks=split_masks
        )
    ...
    def generate_split_masks(
        ...
        graph_df = pd.read_csv(
            "./{}/ml_{}.csv".format(self.base_directory, dataset_name)
        )
        ...
        if save_indices:
            ...
            # split_masks
            np.save(
                "./{}/split_index/ml_{}_val_index.npy".format(
                    self.base_directory, dataset_name
                ),
                train_mask,
            )
            np.save(
                "./{}/split_index/ml_{}_test_index.npy".format(
                    self.base_directory, dataset_name
                ),
                train_mask,
            )
            np.save(
                "./{}/split_index/ml_{}_new_node_val_index.npy".format(
                    self.base_directory, dataset_name
                ),
                train_mask,
            )
            np.save(
                "./{}/split_index/ml_{}_new_node_test_index.npy".format(
                    self.base_directory, dataset_name
                ),
                train_mask,
            )
    ...
    def get_data_link_pred_from_indices(
        self, dataset_name, split_masks=None, randomize_fatures=False
    ):
        """
        ...
        if split_masks is not None:
            ...
            train_mask = np.load(
                "./{}/split_index/ml_{}_train_index.npy".format(
                    self.base_directory, self.data_str
                )
            )
            val_mask = np.load(
                "./{}/split_index/ml_{}_val_index.npy".format(
