...
import time
import logging
from evaluation.evaluation_LP import *



def train_val_LP(model, train_val_data, train_val_sampler, n_neigbors, partial_ngh_finder, full_ngh_finder, 
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(0, num_batch, BACKPROP_EVERY):
            ...
            for j in range(BACKPROP_EVERY):
                ...
                model = model.train()
                pos_e = True
                pos_prob = model.compute_edge_probabilities_modified(sources_batch, destinations_batch,
        ...
        # Transductive 
        val_measures_dict = eval_link_pred(model=model, sampler=val_rand_sampler, data=val_data, logger=logger, 
        ...
        # Early stopping
        if early_stopper.early_stop_check(val_measures_dict['ap']):
